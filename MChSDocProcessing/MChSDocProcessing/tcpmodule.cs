...
namespace MChSDocProcessing
{
    ...
    public class TcpModule
    {
        ...
        Dictionary<string, TcpClientData> _tcpClientDatas = new Dictionary<string, TcpClientData>();
        public int who;
        //private Dictionary
        ...
        public void Armname(string neme)
        {
            ArmNames = neme.Split("-");
        }
        /// <summary>
        ...
        public void StopServer()
        {
            if (modeNetwork == Mode.Server)
            {
                ...
                //int number = 1;
                ICollection<string> _tcpClientDatasKey = _tcpClientDatas.Keys;
                foreach (string elem in _tcpClientDatasKey)
                {
                    DeleteClient(_tcpClientDatas[elem]);
                    //number += 1;
                }
        ...
        public void ConnectClient(string ipserver, string ARMname)
        {
            if (recon == 1)
            {
                modeNetwork = Mode.indeterminately;
        ...
        public void SendText(TcpClientData client, string mese)
        {
            SendInfo si = new SendInfo();
            si.message = mese;
            if (String.IsNullOrEmpty(si.message) == true) return;
        ...
        public void SendText(string mese)
        {
            SendInfo si = new SendInfo();
            si.message = mese;
            if (String.IsNullOrEmpty(si.message) == true) return;
            ...
            else if (who < 1)
            {
                ...
                ICollection<string> _tcpClientDatasKey = _tcpClientDatas.Keys;
                TcpClientData _tcp_Client;
                foreach (string elem in _tcpClientDatasKey)
                {
                    _tcp_Client = _tcpClientDatas[elem];
                    if ((!(_tcp_Client == null)) && who < 1)
                    {
                        foreach (string arm in ArmNames)
                        {
                            if (arm == elem)
                            {
                                NetworkStream ns = _tcpClientDatas[elem].tcpClient.GetStream();
                                // Так как данный метод вызывается в отдельном потоке рациональней использовать синхронный метод отправки
                                ns.Write(total, 0, total.Length);
                                //Parent.labelFileName.Text = "";
                                GC.Collect();
        ...
        public void addarm(string namearm)
        {
            ICollection<string> _tcpClientDatasKey = _tcpClientDatas.Keys;
            foreach (string elem in _tcpClientDatasKey)
            {
                if (elem == namearm)
                {
                    SendText(_tcpClient, "busy");
                    DeleteClient(_tcpClient);
                    _tcpClient = null;
                    //_tcpClient.tcpClient.GetStream().Close();
                    //_tcpClient.tcpClient.Close();
                    return;
                }
            }
            _tcpClientDatas.Add(namearm, _tcpClient);
            SendText(_tcpClient, "free");
        }
